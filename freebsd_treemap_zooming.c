...
#ifndef CapSquare
#define CapSquare CapProjecting
#endif
...
// Dynamic filter threshold: tracks how many largest blocks are currently hidden
static int removed_blocks_count = 0;

static long total_sys_pages = 0;
static long page_size = 4096;

// Zoom state (-1 = overview, >= 0 zoomed focus on specific node)
...
static void collect_memory_data(void) {
    size_t len;
    ...
    if (wire_c > 0) {
        ...
        strcpy(nodes[node_count].color_hex, "#1E293B");
        node_count++;
    }
    ...
    for (int i = 0; i < nprocs && node_count < MAX_NODES - 2; i++) {
        ...
        if (!found) {
            ...
            nodes[node_count].count = 1;
            strcpy(nodes[node_count].color_hex, treemap_colors[node_count % PALETTE_SIZE]);
            node_count++;
        }
    }
    kvm_close(kd);
    ...
    if (free_c > 0 && node_count < MAX_NODES) {
        ...
        nodes[node_count].count = 1;
        strcpy(nodes[node_count].color_hex, "#15803D");
        node_count++;
    }
    ...
    for (int i = 0; i < node_count - 1; i++) {
        for (int j = i + 1; j < node_count; j++) {
            if (nodes[j].pages > nodes[i].pages) {
                RectNode tmp = nodes[i];
                nodes[i] = nodes[j];
                nodes[j] = tmp;
            }
        }
    ...
    // Clamp removed count if nodes list shrunk
...
// Bounded Treemap Layout Engine
static void layout_treemap(int start_idx, int end_idx, double x, double y, double w, double h, double area_pages) {
    if (start_idx >= end_idx || area_pages <= 0 || w <= 0.5 || h <= 0.5) return;
    ...
    for (int i = start_idx; i < end_idx; i++) {
        current_sum += nodes[i].pages;
    }
    if (current_sum <= 0) return;

    double half = current_sum / 2.0;
    double accumulated = 0;
    int split = start_idx + 1;

    for (int i = start_idx; i < end_idx - 1; i++) {
        accumulated += nodes[i].pages;
        if (accumulated >= half) {
            split = i + 1;
            break;
        }
    ...
    double left_pages = 0;
    for (int i = start_idx; i < split; i++) left_pages += nodes[i].pages;

    double ratio = left_pages / current_sum;

    if (w > h) {
        double w_left = w * ratio;
        layout_treemap(start_idx, split, x, y, w_left, h, left_pages);
        layout_treemap(split, end_idx, x + w_left, y, w - w_left, h, current_sum - left_pages);
    } else {
        double h_top = h * ratio;
        layout_treemap(start_idx, split, x, y, w, h_top, left_pages);
        layout_treemap(split, end_idx, x, y + h_top, w, h - h_top, current_sum - left_pages);
    }
}

// Renders lower-left Help Controls Overlay
static void draw_help_legend(Display *dpy, Window win, GC gc, int win_h) {
    ...
    if (leg_y < 10) return; // Hide if window is too short

    // Legend Box Background
    XParseColor(dpy, cmap, "#0F172A", &xcolor); // Dark slate
    XAllocColor(dpy, cmap, &xcolor);
    XSetForeground(dpy, gc, xcolor.pixel);
    XFillRectangle(dpy, win, gc, leg_x, leg_y, leg_w, leg_h);

    // Legend Border
    XSetForeground(dpy, gc, WhitePixel(dpy, DefaultScreen(dpy)));
    XDrawRectangle(dpy, win, gc, leg_x, leg_y, leg_w, leg_h);

    // Text Instructions
    const char *title = "[ CONTROLS LEGEND ]";
    const char *line1 = " - : Remove largest memory block from view";
    const char *line2 = " + : Restore hidden memory block to view";
    const char *line3 = " L-Click : Zoom block";
    const char *line4 = " R-Click/ESC : Reset Zoom";
    const char *line5 = " Q : Quit Application";

    XDrawString(dpy, win, gc, leg_x + 10, leg_y + 16, title, strlen(title));
    XDrawString(dpy, win, gc, leg_x + 10, leg_y + 31, line1, strlen(line1));
    XDrawString(dpy, win, gc, leg_x + 10, leg_y + 45, line2, strlen(line2));
    XDrawString(dpy, win, gc, leg_x + 10, leg_y + 59, line3, strlen(line3));
    XDrawString(dpy, win, gc, leg_x + 10, leg_y + 73, line4, strlen(line4));
    XDrawString(dpy, win, gc, leg_x + 10, leg_y + 87, line5, strlen(line5));
}

static void draw_treemap(Display *dpy, Window win, GC gc, int win_w, int win_h, int mx, int my) {
    ...
    for (int i = 0; i < node_count; i++) {
        nodes[i].x = nodes[i].y = nodes[i].w = nodes[i].h = 0;
    }
    ...
    if (zoomed_node_idx >= 0 && zoomed_node_idx < node_count) {
        ...
        XParseColor(dpy, cmap, nodes[idx].color_hex, &xcolor);
        XAllocColor(dpy, cmap, &xcolor);
        XSetForeground(dpy, gc, xcolor.pixel);
        XFillRectangle(dpy, win, gc, 0, 0, win_w, win_h);

        XSetForeground(dpy, gc, BlackPixel(dpy, DefaultScreen(dpy)));
        XSetLineAttributes(dpy, gc, 3, LineSolid, CapSquare, JoinMiter);
        XDrawRectangle(dpy, win, gc, 1, 1, win_w - 3, win_h - 3);
        XSetLineAttributes(dpy, gc, 1, LineSolid, CapSquare, JoinMiter);
        ...
        XSetForeground(dpy, gc, BlackPixel(dpy, DefaultScreen(dpy)));
        XFillRectangle(dpy, win, gc, 10, 10, 480, 25);
        XSetForeground(dpy, gc, WhitePixel(dpy, DefaultScreen(dpy)));
        XDrawString(dpy, win, gc, 20, 27, banner, strlen(banner));

        hovered_idx = idx;
    } else {
        double visible_pages = 0;
        for (int i = active_start_idx; i < node_count; i++) {
            visible_pages += nodes[i].pages;
        }

        layout_treemap(active_start_idx, node_count, 0.0, 0.0, (double)win_w, (double)win_h, visible_pages);
        ...
        for (int i = active_start_idx; i < node_count; i++) {
            ...
            // Fill Block
            XParseColor(dpy, cmap, nodes[i].color_hex, &xcolor);
            XAllocColor(dpy, cmap, &xcolor);
            XSetForeground(dpy, gc, xcolor.pixel);
            XFillRectangle(dpy, win, gc, nodes[i].x, nodes[i].y, nodes[i].w, nodes[i].h);

            // Black Outlines
            XSetForeground(dpy, gc, BlackPixel(dpy, DefaultScreen(dpy)));
            XDrawRectangle(dpy, win, gc, nodes[i].x, nodes[i].y, nodes[i].w - 1, nodes[i].h - 1);
            if (nodes[i].w > 4 && nodes[i].h > 4) {
                XDrawRectangle(dpy, win, gc, nodes[i].x + 1, nodes[i].y + 1, nodes[i].w - 3, nodes[i].h - 3);
            }
            ...
            if (nodes[i].w > 45 && nodes[i].h > 20) {
                ...
                XSetForeground(dpy, gc, WhitePixel(dpy, DefaultScreen(dpy)));
                XDrawString(dpy, win, gc, nodes[i].x + 6, nodes[i].y + 16, txt, strlen(txt));
            }
        }
        ...
        if (removed_blocks_count > 0) {
            ...
            XSetForeground(dpy, gc, WhitePixel(dpy, DefaultScreen(dpy)));
            XDrawString(dpy, win, gc, 12, 19, status, strlen(status));
        }
    }

    // Always draw persistent help legend overlay
    draw_help_legend(dpy, win, gc, win_h);
    ...
    if (hovered_idx != -1) {
        ...
        XSetForeground(dpy, gc, BlackPixel(dpy, DefaultScreen(dpy)));
        XFillRectangle(dpy, win, gc, tip_x + 3, tip_y + 3, tip_w, tip_h);

        XParseColor(dpy, cmap, "#FEF08A", &xcolor);
        XAllocColor(dpy, cmap, &xcolor);
        XSetForeground(dpy, gc, xcolor.pixel);
        XFillRectangle(dpy, win, gc, tip_x, tip_y, tip_w, tip_h);

        XSetForeground(dpy, gc, BlackPixel(dpy, DefaultScreen(dpy)));
        XDrawRectangle(dpy, win, gc, tip_x, tip_y, tip_w, tip_h);

        XDrawString(dpy, win, gc, tip_x + 8, tip_y + 16, line1, strlen(line1));
        XDrawString(dpy, win, gc, tip_x + 8, tip_y + 32, line2, strlen(line2));
        XDrawString(dpy, win, gc, tip_x + 8, tip_y + 48, line3, strlen(line3));
    }
}
    ...
    while (1) {
        while (XPending(dpy)) {
            ...
            if (ev.type == ConfigureNotify) {
                ...
                draw_treemap(dpy, win, gc, win_w, win_h, mouse_x, mouse_y);
                ...
                if (ev.xbutton.button == Button1) {
                    for (int i = 0; i < node_count; i++) {
                        if (ev.xbutton.x >= nodes[i].x && ev.xbutton.x < (nodes[i].x + nodes[i].w) &&
                            ...
                            zoomed_node_idx = i;
                            draw_treemap(dpy, win, gc, win_w, win_h, ev.xbutton.x, ev.xbutton.y);
                            break;
                        }
                    }
